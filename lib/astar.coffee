	...
	pathfinder =
		...
		search: (matrix, start, end) ->
			...
			###
				...
				# End case: if result has been found, return the traced path
				if currentNode is endNode
					curr = currentNode
					ret = []
				...
				while i < il
					...
					if !beenVisited or gScore < neighbour.g
						...
						neighbour.visited = true
						neighbour.parent = currentNode
						neighbour.h = neighbour.h or
						@manhattan([neighbour.x, neighbour.y], [end.x, end.y])
						neighbour.g = gScore
						neighbour.f = neighbour.g + neighbour.h
		...
		while openHeap.size() > 0
			...
			while i < il
				...
				if !beenVisited or gScore < neighbour.g
					...
					neighbour.visited = true
					neighbour.parent = currentNode
					neighbour.h = neighbour.h or @manhattan(neighbour.pos, end.pos)
					neighbour.g = gScore
					neighbour.f = neighbour.g + neighbour.h
